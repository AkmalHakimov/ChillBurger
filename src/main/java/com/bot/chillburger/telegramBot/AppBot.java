...
public class AppBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            ...
            if (message.hasText()) {
                ...
                if (text.equals("/start")) {
                    ...
                    if (text.equals(ShowBotMessage(BotMessage.BYD_BTN_MSG))) {
                        ...
                        saveTelegramUserToDb();
                    }else if(text.equals(ShowBotMessage(BotMessage.ORDERS_BTN_MSG))){
                        showBasket(chatId);
                    ...
                    if(text.equals(ShowBotMessage(BotMessage.BACK_BTN_MSG))){
                        interactiveMenuSection(chatId,message);
                    }else if(text.equals(ShowBotMessage(BotMessage.CLEAN_BTN_MSG))){
                        orderProductRepo.deleteRedundantOrderProducts(telegramUser.getId());
                        interactiveMenuSection(chatId,message);
                    }else if(text.equals(ShowBotMessage(BotMessage.CONTINUE_BTN_MSG))){
                        SendMessage sendMessage = new SendMessage(chatId.toString(),ShowBotMessage(BotMessage.CONFIRMED_MSG));
            ...
            if (telegramUser.getState().equals(BotState.CHOOSE_LANG)) {
               ...
               if (data.equals(BotCallBackData.GO_TO_MENU.toString())) {
                    menuButtons(chatId);
                }
            }
        }
    ...
    private String getCaption(Product product) {
        if(telegramUser.getSelectedLang().equals("SET_LANG_UZB")){
    ...
    private String returnProductTypeText(ProductType currentProductType) {
            if(currentProductType==ProductType.THIN){
                return ShowBotMessage(BotMessage.THIN);
            }else if(currentProductType==ProductType.THICK){
                return ShowBotMessage(BotMessage.THICK);
            }else {
                return ShowBotMessage(BotMessage.HOT_DOG_BURGER);
            }
    }

    private String returnProductSizeText(ProductSize productSize) {
        if(productSize==ProductSize.SMALL){
            return "маленькая";
